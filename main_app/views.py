...
def SportPrediction():
    ...
    "Both teams to score",
    "Draw(no team to score)",

    "Both teams to score and team a win",
    "Both teams to score and team b win",
    ...
    "Team a wins 1st half",
    "Team b wins 1st half",

    "Team A wins 1st half and wins the game",
    "Team B wins 1st half and wins the game",
    "Team A wins 1st half and loses the game",
    "Team B wins 1st half and loses the game",

    "Team A wins or draws",
    "Team B wins or draws",

    "Team A loses or draws",
    "Team B loses or draws",

]
    tomorrow_date = datetime.date.today() + datetime.timedelta(days=1)
    url = "https://onefootball.com/en/matches?date=%s"%(tomorrow_date)

    page = requests.get(url)
    soup = BeautifulSoup(page.text, 'html.parser')

    spans = soup.find_all("span", {"class": "title-8-medium simple-match-card-team__name"})
    imgs = soup.find_all("img", {"width": "22"})
    ...
    for item in imgs:
        item = str(item).split()
        logos.append(item[-2][4:])


    predictions = []
    for item in range(len(players)):

        prediction_index = random.randint(0, len(possible_predictions)-1)
        likelyhood = possible_predictions[prediction_index]

        predictions.append(likelyhood)


    data = zip(players, logos, predictions)

    response = []

    count = 0
    for player, logo, prediction in data:
        pre_response = {}
        
        if (count % 2) == 0:
            pre_response["home"] = player
            pre_response["home_logo"] = logo
            pre_response["away"] = players[count+1]
            pre_response["away_logo"] = logos[count+1]
            pre_response["prediction"] = prediction

        else:
            pass

        response.append(pre_response)
        count += 1

    response.append(str(datetime.date.today()))
...
def index(request):
    ...
    context = {"data": data[0:-2], 'ads': ads, 'big':big, 'side':side, 'sides':sides, 'footer':footer, 'games':games, "sent":sent}
...
def vip_game(request):
    ...
    try:
        sub = Subscription.objects.filter(user=request.user).last()

        switch_date = sub.switch_date
        today_date = datetime.datetime.now()

        switch_date = int(str(switch_date)[:4]*10) + (int(str(switch_date)[5:7])*10) + int(str(switch_date)[8:10])
        today_date = int(str(today_date)[:4]*10) + (int(str(today_date)[5:7])*10) + int(str(today_date)[8:10])
    ...
    context = {"switch_status": switch_status, "sub": sub, 'games': games, "footer":footer, "sent":sent}
...
def subscribe_thanks(request):
    footer = Advert.objects.all().order_by('-id')[1:9]
    context = {"footer":footer}
    return render(request, 'main_app/subscribe_thanks.html', context)
...
def subscribe2(request):
    ...
    if request.method == 'POST':
        ...
        subscription = Subscription.objects.create(user=request.user, txn_hash=txn_hash, payment_status=True)
        subscription.save()
        ...
        subscription.switch_date = nextD
        subscription.save()

        messages.success(request, "Subscription completed successfully.")
        return redirect(reverse('subscribe_thanks'))
    ...
    context = {"footer":footer}
    return render(request, 'main_app/confirm_payment.html', context)
...
def sub(request):
    if request.method == 'POST':
        ...
        subs = Subscription.objects.order_by("-pub_date")
        context = {"subs": subs}
        return render(request, 'main_app/subs.html', context)
...
def sub_detail(request, sub_id):
    if request.method == 'POST':
        ...
        if approval_status == "activate":
            sub.approval_status = True
        elif approval_status == "deactivate":
            sub.approval_status = False
        else:
            pass

        sub.save()

        messages.success(request, "Subscription Status Updated!")
        ...
        sub = Subscription.objects.get(id=sub_id)
        context = {"sub": sub}
        return render(request, 'main_app/sub_detail.html', context)
